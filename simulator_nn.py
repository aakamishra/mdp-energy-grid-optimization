...
class ParameterizedMDP:
    ...
    def feature_vector(self, state, action):
        ...
        return np.array([
            demand/50000,
            generation/50000,
            min(0, 1/(demand/50000)),
            min(0, 1/(generation/50000)),
            min(0, 1/(action/1750)),
            action/1750,
            battery/1750,
        ...
        for episode in tqdm(range(episodes)):
            ...
            while True:
                ...
                if np.random.random() > self.randomization:
                    q_array = [self.calculate_reward(state, a) for a in action_space]
                ...
                # if simulator.current_time == simulator.data_source.index[-2000]:
                #     pdb.set_trace()
    ...
    def calculate_reward(state, action):
        ...
        if action >= 0:
            ...
            if storable_energy-action < 0:
                reward -= abs(storable_energy-action)*0.5 # penalize over usage attempt of battery
        elif action < 0:
            # Limit the usage amount to available battery storage
            usable_energy = min(abs(action), battery_storage)
            reward = -abs(demand - generation - usable_energy)*2.0  # Penalize unmet demand
            if usable_energy + action < 0:
                #t over", usable_energy, action)
                reward -= abs(usable_energy + action)*0.5 # penalize over usage attempt of battery
        
        reward -= max(0, 1750 - battery_storage)*0.5  # Penalize battery under-utlization
...
class NeuralNetworkMDP:
    def __init__(self, feature_size, action_space, learning_rate=0.001, discount_factor=0.9):
        ...
        self.q_network = QNetwork(feature_size, 128, 1)
    ...
    def update_parameters(self, state, action, reward, next_state):
        ...
        loss = self.criterion(predicted_q, torch.tensor(target).float())
    ...
    def calculate_reward(state, action):
        ...
        if action >= 0:
            # Limit the storage amount to the available over-generation and battery capacity
            storable_energy = min(action, over_generation, 1750 - battery_storage)
            reward = -abs(demand - generation + storable_energy)  # Penalize unmet demand
            if storable_energy-action < 0:
                reward -= abs(storable_energy-action) # penalize over usage attempt of battery
        elif action < 0:
            # Limit the usage amount to available battery storage
            usable_energy = min(abs(action), battery_storage)
            reward = -abs(demand - generation - usable_energy)  # Penalize unmet demand
            if usable_energy + action < 0:
    ...
    def train(self, simulator, episodes, epsilon=0.1):
        """
        ...
        for episode in tqdm(range(episodes)):
            simulator = CampusEnergyReplaySimulator()  # Reset simulator
            total_reward = 0
            ...
            while True:
                ...
                if np.random.random() < epsilon:
                    ...
                    q_values = [self.q_value(state, action).item() for action in self.action_space]
                    action = self.action_space[np.argmax(q_values)]  # Exploit
                    # print("policy-action", action)

                reward = self.calculate_reward(state, action)  # Compute reward
                simulator.next_step(action)  # Take action
                ...
                # Update Q-network
                loss = self.update_parameters(state, action, reward, next_state)
...
if __name__ == "__main__":
    feature_size = 15  # Example feature size
    learning_rate = 0.05
    discount_factor = 0.6
    num_episodes = 5
    ...
    mdp = NeuralNetworkMDP(feature_size, action_space, learning_rate, discount_factor)
    # simulator = CampusEnergyReplaySimulator()
    # rewards = mdp.train(simulator, episodes=num_episodes, epsilon=epsilon)

    # current_time = str(time.time())
    # torch.save(mdp.q_network.state_dict(), f"q_network{current_time}.pth")
    ...
    # trained_parameters_path = "weights1732931256.74818.npy"
    # mdp = ParameterizedMDP(feature_size, learning_rate, discount_factor)
    # mdp.theta = np.load(trained_parameters_path)

    trained_model_path = "q_network1733091153.047692.pth"
    mdp.q_network.load_state_dict(torch.load(trained_model_path))
    mdp.q_network.eval()

    simulator = CampusEnergyReplaySimulator()
    # Run inference
    total_reward, action_log, timestamps = mdp.run_inference(simulator)
